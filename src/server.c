...
int http_send_response(int connectionfd, char *message, FILE *infp, http_version_t version,
                       http_status_code_t status,
                       http_content_type_t content_type,
                       int connection_keep_alive, struct timeval start_time);
int http_send_bad_request(int connectionfd, http_version_t version, struct timeval start_time, char *message, ...);

http_method http_get_method(char *method)
{
    for (size_t i = 0; i < sizeof(HTTP_METHODS_SUPPORTED) / sizeof(char *); i++)
    {
...
http_version_t http_get_version(char *version)
{
    for (size_t i = 0; i < sizeof(HTTP_VERSIONS_SUPPORTED) / sizeof(char *); i++)
    {
...
http_content_type_t http_get_content_type(char *file_name)
{
    ...
    if (!file_ext)
        return HTTP_CONTENT_TYPE_INVALID;
    for (size_t i = 0; i < sizeof(HTTP_CONTENT_TYPES_EXT) / sizeof(char *); i++)
    {
...
char *http_get_status_message(http_status_code_t status)
{
    for (size_t i = 0; i < sizeof(HTTP_STATUS_CODES_MAP) / sizeof(char *); i++)
    {
                       ...
                       int connection_keep_alive, struct timeval start_time)
{
    ...
    {
        sprintf(response_packet + strlen(response_packet), "Content-Type: %s\r\n",
...
void sig_handler(int sig)
{
    if (sig == SIGINT)
    {
        ...
        {
            // Set a flag to indicate that the child process should exit after the current request
            child_exit = 1;
        }
    }
    else if (sig == SIGCHLD)
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        else if (pid == 0)
        {
            ...
            hostp = gethostbyaddr((const char *)&((struct sockaddr_in *)&client_addr)->sin_addr.s_addr,
                                  sizeof(((struct sockaddr_in *)&client_addr)->sin_addr.s_addr), AF_INET);
            if (hostp == NULL)
            {
            ...
            while (!child_exit)
            {
                ...
                // Read from the file descriptor until we reach the end of the header, we read too much data, or we get an error such as a timeout
                while (!received_header && header_len < MAX_REQ_SIZE)
                {
                    ...
                    for (i = 0; i < header_len - 3; i++)
                    {
                            ...
                            header[i + 2] == '\r' && header[i + 3] == '\n')
                        {
                            ...
                            // We guarenteed an extra space for the null terminator
                            // by adding 1 to the header buffer size
                            if (i + 4 < MAX_REQ_SIZE)
                            {
